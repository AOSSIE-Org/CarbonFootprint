class TrainsFootprintCore {
  ...
  storeDataSource(dataSource) {
    this.getData(
      this.helper.getFilePath("core/resources/trainEmissions.json"),
      data => {
   ...
   *  @param String
   */
  storeTrainSpeed(dataSource) {
    this.getData(this.helper.getFilePath("core/resources/trains.json"), data => {
      ...
      data1 => {
        ...
        self.getData(
          ...
          data2 => {
            ...
            distanceBetween = self.getDistance(
              depart.lat,
              depart.lng,
              arrive.lat,
              arrive.lng
  ...
  */
  convertToTrainDistance(distance) {
  ...
  getEmission(modeList) {
    ...
    for (var y = 0, j = modeList.length; y < j; y++) {
      ...
      modeAverage += trainData[modeList[y]]
        ? trainData[modeList[y]]
        : trainData.average;
    }
