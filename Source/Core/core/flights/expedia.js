class expediaManager {
  ...
  getList() {
    ...
    var rawList = document.getElementsByClassName(
      "flight-module segment offer-listing"
    );
    ...
    for (var x = 0; x < rawList.length; x++) {
      details = this.validator.getByClass(
        "secondary-content no-wrap",
        rawList[x]
      )[0];
      depart = this.validator
        .querySelector(".visuallyhidden", details)
      ...
      stops = this.validator
        .getByClass("layover-stops", details[1])[0]
        .dataset["stopLayover"].split(",");
  ...
  insertInDom(processedList) {
    if (processedList.length) {
      var checkOption = this.validator.getByClass(
        "flight-module segment offer-listing"
      );
