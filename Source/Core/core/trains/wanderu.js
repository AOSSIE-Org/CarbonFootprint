var busData, busEmissionData;
var wanderuManager = function(footprintCore, settingsProvider) {
  ...
  this.footprintCore.getData(
    this.footprintCore.helper.getFilePath("core/resources/busesEmissions.json"),
    function(data) {
...
wanderuManager.prototype.setStyle = function(emission) {
  emission.style.margin = "0 1em";
  emission.style.display = "inline";
  return emission;
};

wanderuManager.prototype.insertInDom = function(emission, element) {
...
wanderuManager.prototype.update = function() {
  if (document.querySelectorAll(".search-result-component").length === 0)
    return;
  var self = this;

  document.querySelectorAll(".search-result-component").forEach(function(row) {
    ...
    if (!row.querySelector(".modesAndDetailsContainer-2UCxL svg")) {
      ...
      startTime = startTime.map(function(e) {
      ...
      endTime = endTime.map(function(e) {
        return parseInt(e, 10);
      });
      ...
      self.insertInDom(self.footprintCore.createHTMLElement(emission), row);
      return;
    }
    ...
    if ((bus || train) && !(bus && train)) {
      ...
      allrows.forEach(function(tr, index) {
        if (tr.querySelector("td:nth-child(3)").textContent) {
          var mode =
            tr.querySelector("td:nth-child(5)").textContent.indexOf("Train") > 0
              ? "train"
              : "bus";
          var startTime = tr
            .querySelector("td:nth-child(3)")
